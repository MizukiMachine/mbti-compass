...
const systemPrompt = `あなたは職場の人間関係に特化したコミュニケーション設計コーチです。

## 方針
- 相手のMBTIは断定せず、観察情報からの仮説として扱う
- 操作や支配ではなく、相互尊重、境界線、合意形成を重視する
- 医療、法務、ハラスメント認定の断定は避け、深刻な危険や違法性がある場合は専門窓口への相談を促す
- 日本語で、職場で実際に使える具体的な言い方に落とす
...
厳密なJSONのみを返す。マークダウンコードブロックは不要。
{
  ...
  "messageDraft": "Slack、メール、または口頭で使える文面",
...
## 具体的な相談内容
${request.userConcern?.trim() || 'まだ具体的な相談内容は未入力。一般的な職場コミュニケーションの次の一手を提案してほしい。'}`;
}

const actionDrafts: Record<AdviceActionId, string> = {
  build_trust:
    'お疲れさまです。最近の進め方について、少し認識を合わせたいです。こちらで意識した方がよいポイントがあれば先に知っておきたいので、短く相談させてください。',
  request:
    'お疲れさまです。お願いしたいことがあります。目的は〇〇で、お願いしたい範囲は△△です。負荷が大きければ、期限や範囲を調整したいので相談させてください。',
  decline:
    'お声がけありがとうございます。現状の優先度と期限を考えると、そのまま引き受けるのは難しそうです。代わりに、〇〇までなら対応できます。',
  feedback:
    '少し共有したいことがあります。責めたいわけではなく、次回もっと進めやすくするための確認です。事実として〇〇があり、次は△△にできると助かります。',
  repair:
    '先ほどの件で、こちらの伝え方が足りなかったかもしれません。一度意図を整理して伝え直したいです。誤解があれば解きたいので、少し時間をください。',
  distance:
    '今後のやり取りを進めやすくするために、確認事項はテキストで残しながら進めたいです。急ぎのものは優先度と期限を添えていただけると助かります。',
  smalltalk:
    '最近のプロジェクト、どのあたりが一番大変ですか。自分も状況を知っておきたいので、差し支えない範囲で教えてください。',
  one_on_one:
    '次の1on1では、進捗、困っている点、相談したい判断の3つを話したいです。特に〇〇について意見をもらえると助かります。',
};
...
export function generateFallbackWorkplaceAdvice(request: WorkplaceAdviceRequest): WorkplaceAdviceResponse {
  ...
  return normalizeAdvice({
    title: `${person.relationLabel}との「${action.label}」方針`,
    ...
    psychologyNote:
      '職場の対立は性格そのものより、重視する価値や情報量の違いから起きることが多いです。相手の価値を先に満たすと、防衛的な反応が下がりやすくなります。',
    caveat:
